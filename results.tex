...
\begin{figure*}[htb]
    ...
    \caption{Average energy score rise over baseline (on $\mathbb{S}_{\infty}^{d-1}$) for various models on simulated data, with ascending count of mixture components (indicated by plot heading) and number of dimensions (indicated by horizontal axis).  Note that pairwise betas is a moment-restricted model.\label{fig:simpples}}
...
We apply the aforementioned models to simulated angular data. We then consider the analysis
    ...
    density. In all the examples considered MCMC samples produced stable tarces of the
    log-posterior in less than 40 thousand  iterations. Thus, so we use that as a burn-in, 
    ...
    posterior predictive densities. We used two different strategies implemment the MCMC samplers.
    For the models whose DP prior is centered around a log-normal distribution we used parallel 
    tempering. For the models gamma-centered models, we used a single chain. The temperature ladder was set as $t_s = 1.3^{s}$, 
    for $s \in \lbrace0,1,\ldots,5$. This was set empirically in order to produce
    acceptable swap probabilities both for the simulated data, and real data. Our attempts to use
    a single chain produced sample with very slow mixing. On the other hand, from our examples 
    we observe that posterior samples from the gamma-centered models, that do not use parallel 
    tempering, show slightly stronger autocorrelation than those of the log-normal centered 
    models. Parallel tempering produces chains with good mixing properties, but has a computational 
    cost that  grows linearly with the number of temperatures. In view of this, we use it for the
...
To evaluate our proposed approach for angular measure estimation 
    ...
    procedure is detailed in Algorithm~\ref{algo:simulated}.  For each
    model, the data is then projected onto the appropriate $\mathbb{S}_p^{d-1}$
    for model fitting--dirichlet and pairwise betas being fit on 
    $\mathbb{S}_1^{d-1}$, and the projected gamma models being fit on 
    $\mathbb{S}_{10}^{d-1}$.

\begin{algorithm}[h]  % algorithm using algorithm package
        ...
        \For{$n_{\text{mix}}$ in $[1, 2, 4, 8]$}
            ...
            \EndFor
            \State Generate $\pi\sim\text{Dirichlet}(\bm{10})$
...
Figure~\ref{fig:simpples} shows the average rise over baseline in energy score as
    ...
    dataset.  We considered a total of four variations of the Dirichlet-process mixture
    ...
    % it is} 
    We observe that for simulated data using Algorithm~\ref{algo:simulated}, the 
    projected gamma models dominate, and within that class of model, the choice of 
    centering distribution is irrelevant.  For the projected \emph{restricted} 
    gamma models, the choice of a multivariate log-normal centering distribution 
    appears to negatively affect model fidelity.
...
Our data corresponds to daily average values for the IVT magnitude
    ...
    correspond to the coast of California.  The ERA5 data covers the time period 
...
% Table~\ref{tab:dev} shows the values of the estimated posterior predictive loss and energy 
%     scores for the different models considered. We observe that, as in the case of the simulation 
%     study shown in Figures~\ref{fig:simpples}, the preferred model is the projected restricted 
%     gamma models, though for the lower-dimensional ERA-Interim data, all models perform comparably.
%     Interestingly, this preference for the restricted gamma form is the opposite of the result 
%     obtained in the simulation example.
Table~\ref{tab:dev} shows the values of the estimated posterior predictive loss and energy 
