...
We apply the aforementioned models to generated angular data, as well as a data analysis
    of atmospheric data.
    Assessing convergence of an MCMC chain for a large-dimensional model is difficult.
    For our part, we assess convergence by stabilization of the log-posterior 
    ...
    Implementation details play a role here: posterior samples from the gamma-centered 
    models are moderately autocorrelated as compared to the log-normal centered models.
    This is a benefit of the parallel tempering algorithm employed in fitting the 
    log-normal models, but parallel tempering necessitates an additional computational cost,
    growing linearly with the number of temperatures.  We used a temperature ladder
    of $t_s = 1.3^{s}$, for $s \in \lbrace0,1,\ldots,5$.  This was verified to produce
    acceptable swap probabilities both for the simulated data, and real data.
...
Our hyperprior parameters are set as follows: for the gamma-centered models (PG--G, PRG--G), 
    ...
    $\sigma\sim\mathcal{G}\left(2,2\right)$.  The choice of $\mathcal{G}(2,2)$ for rate
    parameters was meant to draw estimates for the value away from 0 for numerical stability.
