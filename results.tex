...
To evaluate our proposed approach for angular measure estimation 
    ...
    projected onto $\mathbb{S}_p^{d-1}$. The generation procedure is detailed 
...
\begin{algorithm}[ht]  % algorithm using algorithm package
        ...
        \For{$n_{\text{mix}}$ in $[1, 2, 4, 8]$}
            \For{$j$ in $1,\ldots,n_{\text{mix}}$}
                \State Generate $\mu_{j} \sim \mathcal{N}\left(\bm{0},\bm{I}\right)$
                \State Generate $\Sigma_{j}\sim\mathcal{IW}\left(70,70 \bm{I}\right)$
            \EndFor
...
where $\mu_j$, $\Sigma_j$ the parameters of the log-normal mixture component 
    distribution; $\pi$ the probability vector assigning weight to each mixture 
    component, and $\delta_i$ the mixture component identifier for each simulated 
    observation.  
...
Figure~\ref{fig:simpples}, shows the average rise over baseline in energy score as
    ...
    projected \emph{restricted} gamma models, the choice of log-normal centering
    distribution induces 





shows two sets of panels that illustrate the comparison
