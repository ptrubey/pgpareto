...
% An alternative to projecting a distribution in ${\mathbb R}_+^d$ to $S_{p}^{d-1}$ would be to map
%   ${\mathbb S}_{p}^{d-1}$ to an alternative geometry---some $\Omega^{d-1}$, and fitting a
...
%   which map $\bm{x}\in{\mathbb R}_+^{d}$ to $(-\infty, \infty)^{d-1}$.  For $p = 2$---the Euclidean
%   sphere---we have spherical coordinates, where we map $\bm{x}\in{\mathbb R}_+^d$ to $[0,\pi/2]^{d-1}$:
%   \begin{equation*}
